...
app.controller('emailBuildCtrl', function ($scope, $http, $location) {
	...
	$scope.createNew = function () {
		if (!$scope.file) {
			...
			$scope.addSection(0, "banner");
			$scope.addSection(1, "story");
		} else {
...
app.controller('imageCtrl', function ($scope, $http) {
	...
	$scope.loadFile = function (file) {
		...
		$scope.imgCreation.x = 250;
		$scope.imgCreation.y = 150;
	...
	$scope.loadCanvas = function () {
		...
		destCtx.drawImage(canvas, -200, -100);
	};
	...
	return {
		link: function (scope, attr, el) {
			...
			$(document.body).mouseup(function (e) {
				scope.releaseImage(e);
			});
			$("#myCanvas").on('mousewheel', function (e) {
				scope.mouseWheelZoom(e);
			});
		}
	};
}]);
